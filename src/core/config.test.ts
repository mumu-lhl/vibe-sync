...
sync_to:
  ...
  - "Roo Code"`;
    ...
    it("should create a new vibesync.yaml if one does not exist", async () => {
      ...
      expect(console.log).toHaveBeenCalledWith(
        chalk.bold.green("vibesync.yaml created successfully!"),
      );
    });
    ...
    it("should not create a new vibesync.yaml if one already exists", async () => {
      ...
      expect(console.log).toHaveBeenCalledWith(
        chalk.yellow("vibesync.yaml already exists. Aborting initialization."),
      );
    });
    ...
    it("should handle errors during file creation", async () => {
      ...
      expect(console.error).toHaveBeenCalledWith(
        chalk.bold.red("Failed to create vibesync.yaml:"),
        error,
      );
    });
  });
  ...
  describe("loadConfig", () => {
    ...
    it("should exit if the config file is not found", () => {
      ...
      // Assert
      expect(console.error).toHaveBeenCalledWith(
        chalk.red(`Error reading or parsing ${configPath}:`),
        error,
      );
