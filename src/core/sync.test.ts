...
describe('Sync Tests', () => {
    ...
    describe('sync function', () => {
        it('should perform a sync from a source to multiple destinations', async () => {
            ...
            const resolvedSource = { name: 'Gemini', path: '/src/gemini', type: 'directory' as const };
            const resolvedDests = [
            ...
            expect(console.log).toHaveBeenCalledWith(chalk.bold.green('\nSync completed successfully!'));
        });
        ...
        it('should handle errors during sync and exit', async () => {
            ...
            expect(console.error).toHaveBeenCalledWith(chalk.bold.red('\nSync failed:'), error);
    ...
    describe('copySourceToDest', () => {
        it('should copy from directory to directory', async () => {
            ...
            expect(fs.cp).toHaveBeenCalledWith(mockSource.path, mockDest.path, { recursive: true, force: true });
        });
        ...
        it('should copy a file to a directory, creating vibesync.md', async () => {
            ...
            expect(fs.cp).toHaveBeenCalledWith(mockSource.path, expectedDestPath, { recursive: true, force: true });
        });
        ...
        it('should merge files from a directory into a single file', async () => {
            ...
            expect(fs.writeFile).toHaveBeenCalledWith(mockDest.path, 'content\ncontent');
            expect(console.log).toHaveBeenCalledWith(chalk.green(`Successfully merged 2 files into ${mockDest.path}`));
        });

        describe('Special Handling', () => {
            it('should handle special sync for Cline as destination', async () => {
                const clineDest = { name: 'Cline', path: '.clinerules/', type: 'directory' as const };
                vi.mocked(config.loadConfig).mockReturnValue({ version: 1, sync_from: 'Gemini', sync_to: ['Cline'] });
                vi.mocked(config.resolveSyncObject)
            ...
            it('should handle special sync for Kilo Code as source', async () => {
                const kiloSource = { name: 'Kilo Code', path: '.kilocode/', type: 'directory' as const };
                vi.mocked(config.loadConfig).mockReturnValue({ version: 1, sync_from: 'Kilo Code', sync_to: ['some_dest'] });
                vi.mocked(config.resolveSyncObject)
            ...
            it('should handle special sync for Roo Code as source', async () => {
                const rooSource = { name: 'Roo Code', path: '.roo/', type: 'directory' as const };
                vi.mocked(config.loadConfig).mockReturnValue({ version: 1, sync_from: 'Roo Code', sync_to: ['some_dest'] });
                vi.mocked(config.resolveSyncObject)
